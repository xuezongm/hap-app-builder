...
> **读取文件时**，使用当前环境可用的文件读取工具：
> - **Antigravity**：`view_file`
> - **Codex**：`exec_command`（如 `sed -n '1,260p' <file>`）
...
>
> 各 IDE 的子 agent 调用方式不同：
> - **Antigravity**：使用 `invoke_subagent` 工具
> - **Codex**：使用 `multi_agent_v1.spawn_agent` 启动子 agent，使用 `multi_agent_v1.wait_agent` 等待结果
> - **Claude Code**：使用 `dispatch_agent` 或 Task 工具
...
## 执行要求
1. 先完整读取步骤文件（Antigravity 用 `view_file`；Codex 用 `exec_command`；Claude Code 用 `read_file`）
