...
> 读取文件时，使用当前环境可用的文件读取工具。在 Antigravity 中使用 `view_file`；在 Codex 中使用 `exec_command`（如 `sed -n '1,260p' <file>`）；在 Claude Code 中使用 `read_file`。
...
Step 3 起数据量和复杂度上升，**优先使用子 agent 隔离执行**。

> [!CAUTION]
> **Step 3~11 的每一步都必须先尝试将任务委派给子 agent。只有当子 agent 功能确实不可用时，才退回内联执行。**
>
> 调用 subagent 时，使用当前环境可用的 subagent 工具。在 Antigravity 中使用 `invoke_subagent`；在 Codex 中使用 `multi_agent_v1.spawn_agent` 启动子 agent，用 `multi_agent_v1.wait_agent` 等待结果；在 Claude Code 中使用 `dispatch_agent` 或 Task 工具。

#### 子 agent 执行流程

对 Step 3~11 的每一步：

1. **尝试将任务委派给子 agent**，使用下方 Prompt 模板
