...
1. **探测子 agent 功能**：尝试检测当前平台是否支持子 agent（如 `invoke_subagent`、`create_task`、`multi_agent_v1.spawn_agent` 等原语），子 agent 工具可能不会出现在初始工具列表中，必须先通过工具发现查询，不得仅因初始工具列表未显示 `spawn_agent` 就判定子 agent 不可用。
2. **根据检测结果分流**：
